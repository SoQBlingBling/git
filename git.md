...
- git add xxx
- git commit -m' xxx'
- git remote add origin  'xxxxxxxxxxxx(http/ssh地址 )'
...
| git checkout -b 新建的分支名               | 新建分支并切换到新分支                                       |
| git branch -d 分支名                       | 删除分支                                                     |
| git branch                                 | 查看所有分支 并且带有* 的为当前所在分支                      |
| git merge 分支名                           | 合并分支                                                     |
| git branch -m \|-M 旧分支名字 新的分支名字 | 重命名分支 如果new branch 名字分支 已经存在 则需要使用 -M强制重命名，否则使用-进行重命名 |
...
| git push origin :remove_branch                    | 删除远程分支（本地分支报存）     |
